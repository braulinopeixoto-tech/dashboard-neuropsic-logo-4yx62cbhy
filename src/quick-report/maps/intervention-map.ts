...
export function generateInterventionPhases(context: NeurofunctionalContext): InterventionPlan {
  ...
  const hasMedicalAlert =
    ...
    context.qeegStructuredMarkers.some((marker) => marker.band === 'delta')
  const shouldStabilizeFirst =
    state.brainEnergy === 'hypoactive' || state.brainEnergy === 'unstable'
  const functions = [
  ...
  const hasExecutiveDemand = functions.some((item) =>
    [
      'atencao sustentada',
      'controle inibitorio',
      'memoria operacional',
  ...
  const hasEmotionDemand =
    functions.includes('regulacao emocional') ||
    functions.includes('tolerancia a frustracao') ||
    functions.includes('hiperalerta')
  ...
  return {
    phase1: [
      ...
      ...(hasMedicalAlert
        ? [
            'Encaminhar para avaliacao medica/neurologica complementar antes de intervencoes intensivas.',
          ]
        : []),
      ...(shouldStabilizeFirst
        ? ['Evitar recomendacao precoce de estimulacao sem estabilizacao da base regulatoria.']
        : []),
    ],
    phase2: [
      'Promover integracao funcional gradual entre redes de regulacao, atencao e controle executivo.',
      ...(hasEmotionDemand
        ? ['Associar psicoterapia, psicoeducacao emocional e manejo ambiental estruturado.']
        : []),
      ...(hasExecutiveDemand
        ? ['Introduzir reabilitacao cognitiva graduada com monitoramento de sobrecarga.']
        : []),
    ...
    phase3: [
      ...
      ...(shouldStabilizeFirst
        ? ['Reavaliar energia cerebral antes de neuromodulacao personalizada.']
        : [
            'Considerar neuromodulacao personalizada somente com biomarcadores e indicacao profissional.',
          ]),
    ],
  }
}
